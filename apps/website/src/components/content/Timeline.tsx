...
const Timeline = ({ items, before, after }: TimelineProps) => (
  <div className="relative z-0 mx-auto max-w-6xl">
    <div
      className={classes(
        `
          absolute left-5 -z-10 w-1 -translate-x-1/2 overflow-clip border-x
          border-alveus-green bg-alveus-green/25
          md:left-1/2
        `,
        before || "top-0",
        after || "-bottom-20",
    ...
    >
      <div className="
        sticky inset-x-0 -top-1 bottom-0 mt-[-50vh] h-[calc(50vh+0.25rem)]
        bg-alveus-green
      " />
    </div>
    <ol>
        ...
        <li key={item.key} className="relative my-4 flex items-start">
          ...
          >
            <div
              className={classes(
                `
                  mt-1 hidden border-y-[1rem] border-solid border-y-transparent
                  md:block
                `,
                idx % 2
            ...
            />
            <div className="
              mt-1 ml-6 border-y-[1rem] border-r-[1rem] border-l-0 border-solid
              border-y-transparent border-r-alveus-green
              md:hidden
            " />
          </div>
