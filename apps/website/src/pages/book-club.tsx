...
const Cover = ({
  ...
  <div
    className={classes(
      `
        relative aspect-2/3 h-auto w-full overflow-hidden rounded-l-sm
        rounded-r-xl bg-alveus-green-900 drop-shadow-lg
      `,
      className,
    )}
  >
    {/* Bookmark (hover) */}
    <div className="
      absolute top-0 right-3 z-20 h-16 w-8 -translate-y-full
      transition-transform
      group-hover:translate-y-0
      group-focus:translate-y-0
    ">
      <div className="
        opacity-0 drop-shadow-md drop-shadow-black/50 transition-opacity
        delay-(--default-transition-duration) duration-0
        group-hover:opacity-100 group-hover:delay-0
        group-focus:opacity-100 group-focus:delay-0
      ">
        <svg
    ...
    {/* Shine (hover) */}
    <div className="
      absolute inset-x-0 top-0 z-10 h-64 max-h-full bg-linear-to-b from-white/20
      to-white/0 opacity-0 transition-opacity
    ...
    {/* Crease */}
    <div className="
      absolute inset-y-0 left-0 w-1.5 bg-linear-to-r from-black/10 via-black/10
      to-white/30
    " />
    <div className="
      absolute inset-y-0 left-1.5 w-1.5 bg-linear-to-r from-white/30 to-white/0
    " />
    <div className="
      absolute inset-y-0 left-2.5 w-1.5 bg-linear-to-r from-black/0 via-black/10
      to-white/10
    " />
    <div className="
      absolute inset-y-0 left-4 w-1.5 bg-linear-to-r from-white/10 via-white/30
      to-white/0
    " />

    {/* Edges */}
    <div className="
      absolute top-0 right-0 left-5 h-2 bg-linear-to-b from-white/20 to-white/0
    " />
    <div className="
      absolute right-0 bottom-0 left-5 h-2 bg-linear-to-t from-white/20
      to-white/0
    " />
    <div className="
      absolute inset-y-0 right-0 w-2 bg-linear-to-l from-white/20 to-white/0
    " />
    ...
    {({ open }) => (
      <>
        <DisclosureButton className="
          group w-full overflow-visible text-start perspective-normal
          focus:outline-hidden
        ">
          <div className="
            origin-[50%_40%] transition-all duration-1000 transform-3d
            group-data-open:mb-[-100%] group-data-open:-translate-y-1/4
            group-data-open:translate-z-2 group-data-open:scale-70
            group-data-open:scale-3d group-data-open:rotate-x-85
          ">
            <Cover
              title={title}
              author={author}
              ...
              className="
                ...
                group-data-open:translate-x-1 group-data-open:-translate-z-0.5
              "
            />
            <div
              className={classes(
                `
                  w-full origin-top -translate-y-0.5 scale-x-95 rotate-x-90
                  rounded-l-xl rounded-r-xs border-4 border-r-0 border-solid
                  bg-linear-to-b from-alveus-tan-50 via-gray-100
                  to-alveus-tan-50 transition-transform duration-1000
                  group-data-open:scale-x-100
                `,
                thickness,
                color,
              )}
          ...
          <Heading
            level={2}
            className="
              relative mt-4 mb-0 transition-[color,font-size,line-height]
              duration-[150ms,1000ms,1000ms]
              group-data-open:text-lg
              group-hover:group-[&:not([data-open])]:text-alveus-green-700
              group-focus:group-[&:not([data-open])]:text-alveus-green-700
            "
          >
    ...
    <>
      ...
      <div className="relative">
        <Image
          ...
          alt=""
          className="
            pointer-events-none absolute -bottom-10 left-0 z-10 hidden h-auto
              ...
              book.type === "placeholder" ? (
                <div key={`placeholder-${i}`} className="mx-auto w-64">
                  ...
                  <Heading level={2} className="relative mt-4 mb-0">
                    <div className="
                      absolute -top-1 left-0 h-1 w-16 bg-alveus-green/50
                    " />
