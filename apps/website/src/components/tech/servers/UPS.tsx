    ...
    <div className="flex h-full">
      ...
      >
        <div
          className={classes(
            "flex",
            size === 1 ? "h-full py-1" : "flex-col rounded-sm bg-gray-400",
          )}
        >
          <div className="aspect-video shrink-0 rounded-sm border-2 border-gray-400 bg-blue-800" />

          {size === 1 ? (
            <div className="m-auto aspect-square h-1/2 rounded-full bg-gray-400" />
          ) : (
            <div className="my-auto flex grow flex-wrap items-center p-px pt-0">
              {Array.from({ length: 4 }).map((_, i) => (
                <div
                  key={i}
                  className="m-px aspect-square grow rounded-sm bg-gray-800"
                />
              ))}
            </div>
          )}
