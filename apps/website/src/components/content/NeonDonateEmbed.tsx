...
<script>
  const observer = new MutationObserver((mutations) => {
    window.parent.postMessage({ type: ${JSON.stringify(`resize-${id}`)}, height: document.body.scrollHeight }, "*");
  });
...
const NeonDonateEmbed = ({ className }: { className?: string }) => {
  ...
  const id = useId();
  const src = useMemo(() => srcDoc(id), [id]);
  ...
  return <iframe ref={ref} srcDoc={src} className={className} />;
};
