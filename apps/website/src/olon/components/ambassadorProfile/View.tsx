...
import { Fragment, type FC, type ReactNode, useMemo, useState } from "react";
...
import { getAmbassadorEpisodes } from "@alveusgg/data/build/animal-quest";
...
import type { AmbassadorProfileData } from "./types";

type Stat = { title: string; value: ReactNode };
...
const getStats = (ambassador: Ambassador): Stats => {
  ...
  return [
      ...
      value: (
        <>
          ...
          <p className="text-base text-alveus-green-700 italic">
            ...
            <Link
              href={`/ambassadors#classification:${convertToSlug(getClassification(species.class))}`}
            >
    ...
    { title: "Native To", value: <p>{species.native}</p> },
    {
    ...
    [
      {
        title: `Date of ${{ live: "Birth", egg: "Hatching", seed: "Planting" }[species.birth]}`,
        value: (
          <>
      ...
      { title: "Sex", value: <p>{ambassador.sex || "Unknown"}</p> },
      {
      ...
      },
      {
        ...
        value: (
          <p>
            <Link
              href={`/ambassadors#enclosures:${camelToKebab(ambassador.enclosure)}`}
            >
              {enclosure.name}
            </Link>
          </p>
...
// Editorial copy (name/alternate/story/mission/fact) from JSON; species, IUCN,
// dates, enclosure, images, clips and Animal Quest are resolved from the key.
export const AmbassadorProfile: FC<{ data: AmbassadorProfileData }> = ({
  data,
}) => {
  const ambassadorKey = data.key as AmbassadorKey;
  const ambassador = ambassadors[ambassadorKey] as Ambassador;
  const images = getAmbassadorImages(ambassadorKey);
  const merchImage = getAmbassadorMerchImage(ambassadorKey);
  const animalQuest = getAmbassadorEpisodes(ambassadorKey);
  ...
  const carouselItems = useMemo(
    ...
    [images, setCarouselLightboxOpen],
  );
    ...
    <>
      ...
      {ambassador.retired && (
        <Section dark className="py-4">
          <Heading level={-1} className="text-center text-2xl text-balance">
            {data.name} retired on {formatPartialDateString(ambassador.retired)}.{" "}
            {ambassador.sex === "Male"
              ? "He is"
      ...
      <div className="relative">
        ...
        >
          ...
          <div className="flex basis-full flex-col py-4 lg:max-w-1/2 lg:basis-1/2 lg:px-8 lg:pt-8">
            <Heading className="text-5xl">{data.name}</Heading>
            {!!data.alternate.length && (
              <p className="-mt-1 mb-2 text-lg text-alveus-green-700 italic">
                also: <span className="text-xl">{data.alternate.join("; ")}</span>
              </p>
            )}

            <div className="my-2 space-y-2 text-xl">
              {[data.story, data.mission]
            ...
            <dl className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-auto-2 lg:grid-cols-auto-2">
              {stats.map((item) => {
                ...
                const entries = nested ? item : [item];
                return (
                  <div
                    key={entries.map((i) => i.title).join(",")}
                    className={classes(
                      "contents",
                      nested &&
            ...
            {data.fact ? (
              <Box dark className="my-6 flex flex-col gap-2 p-4">
                <Heading level={2} id="did-you-know" link className="my-0 text-2xl">
                  Did you know?
                </Heading>
                {data.fact.split("\n\n").map((fact, idx) => (
      ...
      {ambassador.clips.length > 0 && (
        <Section dark className="bg-alveus-green-800">
          <Heading level={2} id="highlights" link className="text-center text-4xl">
            {data.name}&apos;s Highlights
          </Heading>
