...
export function DashboardTrendCharts() {
  ...
  // Calculate totals
  const totalDonations = chartData.data.reduce(
    (sum, d) => sum + d.donations,
    0,
  );
  const totalDonationAmount = chartData.data.reduce(
    (sum, d) => sum + d.donationAmount,
    0,
  );
  const totalUsers = chartData.data.reduce((sum, d) => sum + d.users, 0);
  ...
  const chartDataFormatted = chartData.data.map((d) => ({
    ...
    donationAmount: d.donationAmount / 100, // Convert cents to dollars
    users: d.users,
